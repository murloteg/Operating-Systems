...
int storage_add(storage_t* storage, char* value) {
    ...
    int sync_primitive_init_status = pthread_spin_init(&new_element->sync_primitive, NULL);
...
void storage_destroy(storage_t* storage) {
    if (storage != NULL) {
        ...
        while (node != NULL) {
            free(node->value);
            int destroy_sync_primitive_status = pthread_spin_destroy(&node->sync_primitive);
