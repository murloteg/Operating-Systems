...
void* swap_thread_routine(void* arg) {
    ...
    srand(time(NULL));
    while (true) {
        pthread_spin_lock(&storage->first->sync_primitive);
        ...
        while (previous_node->next != NULL){
            if ((rand() % TOTAL_NUMBER_OF_VARIANTS) == 0) {
                ...
                pthread_spin_lock(&current_node->sync_primitive);
                next_node = current_node->next;

                if (next_node == NULL) {
                    pthread_spin_unlock(&current_node->sync_primitive);
                    break;
                }
                pthread_spin_lock(&next_node->sync_primitive);
                previous_node->next = next_node;
                pthread_spin_unlock(&previous_node->sync_primitive);
...
void* increase_length_finder(void* arg) {
    ...
    while (true) {
        ...
        while (previous_node->next != NULL) {
            ...
            pthread_spin_lock(&current->sync_primitive);
            pthread_spin_unlock(&previous_node->sync_primitive);
